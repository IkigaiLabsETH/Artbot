...
if [ -z "$ANTHROPIC_API_KEY" ] && [ -z "$OPENAI_API_KEY" ]; then
    if [ -f .env ]; then
        ...
        echo -e "${RED}Error: Neither ANTHROPIC_API_KEY nor OPENAI_API_KEY is set.${NC}"
        echo -e "Please set at least one of these environment variables or create a .env file.${NC}"
        exit 1
    fi
fi
