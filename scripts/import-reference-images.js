...
async function importImages() {
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    const promptUsed = await question('Prompt used (if AI-generated, optional): ');
...
async function generateCatalog() {
  ...
  try {
    ...
    execSync('node -e "const { buildReferenceImagesCatalog } = require(\'./dist/utils/reference_images_catalog.js\'); buildReferenceImagesCatalog().catch(console.error);"', { stdio: 'inherit' });
